...
export async function sendProofReminder(db: Pool, client: GiveawayClient) {
    ...
    for(let key of pending_review.rows) {
        ...
        await client.log(`${user.username} \`${user.id}\` failed to upload proof of review for the handout \`${key.id}\` within 7 days`, [], components)
        await user.send({content: `You have not uploaded proof of review for the handout \`${key.id}\` within 7 days.`, components}).catch(() => null)
